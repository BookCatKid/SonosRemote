...
String Sonos::extractXmlValue(const String& xml, const String& tag, bool isEscaped) {
    String startTag = isEscaped ? "&lt;" + tag + "&gt;" : "<" + tag + ">";
    String endTag = isEscaped ? "&lt;/" + tag + "&gt;" : "</" + tag + ">";
    ...
    // Simple unescape for common entities if it was an escaped tag
    if (isEscaped) {
        value.replace("&amp;", "&");
        value.replace("&lt;", "<");
        value.replace("&gt;", ">");
...
// Track information
SonosResult Sonos::getTrackInfo(const String& deviceIP, String& title, String& artist, String& album, int& duration) {
    if (!_initialized) return SonosResult::ERROR_INVALID_DEVICE;
    ...
    if (result == SonosResult::SUCCESS) {
        ...
        if (metadata.length() > 0) {
            // Metadata is often escaped XML (using &lt; and &gt;)
            // Try both escaped and unescaped tags
            title = extractXmlValue(metadata, "dc:title");
            if (title.length() == 0) title = extractXmlValue(metadata, "dc:title", true);
            
            artist = extractXmlValue(metadata, "dc:creator");
            if (artist.length() == 0) artist = extractXmlValue(metadata, "dc:creator", true);
            
            album = extractXmlValue(metadata, "upnp:album");
            if (album.length() == 0) album = extractXmlValue(metadata, "upnp:album", true);
        }
        ...
        logMessage("Track info: " + title + " by " + artist);
    }
